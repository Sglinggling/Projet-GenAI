...
def analyze_semantic_breakdown(top3_df, model, free_text, mood, preferred_genres):
    ...
    emb_genre = model.encode(genre_text, convert_to_tensor=True).to("cpu")
    emb_mood = model.encode(mood, convert_to_tensor=True).to("cpu")
    emb_text = model.encode(free_text, convert_to_tensor=True).to("cpu")
   
    movie_texts = top3_df["semantic_text"].tolist()
    emb_movies = model.encode(movie_texts, convert_to_tensor=True).to("cpu")
   
    scores_genre = util.cos_sim(emb_genre, emb_movies)[0].numpy()
    scores_mood = util.cos_sim(emb_mood, emb_movies)[0].numpy()
    scores_text = util.cos_sim(emb_text, emb_movies)[0].numpy()
...
def main():
    ...
    # 2. Calcul des scores sur TOUTE la base (Forcé sur CPU pour compatibilité)
